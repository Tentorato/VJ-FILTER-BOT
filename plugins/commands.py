...
async def start(client, message):
    ...
    if len(message.command) != 2:
        ...
        reply_markup = InlineKeyboardMarkup(buttons)
        m=await message.reply_sticker("CAACAgUAAxkBAAIgmmebwA24L3ezfwJ1a7T4CUqvYrFhAAJxAAP-U9EUONRZUXWRNUUeBA") 
        ...
        try:
            ...
            if btn:
                ...
                if message.command[1]:
                    ...
                    btn.append([InlineKeyboardButton("♻️ Try Again ♻️", callbackquery.answer ("Join Update Channel Then Try Again...☠️", show_alert = True))])
    ...
    if data.split("-", 1)[0] == "ZoneFlixTv":
        user_id = int(data.split("-", 1)[1])
        zoneflixtv = await referal_add_user(user_id, message.from_user.id)
        if zoneflixtv and PREMIUM_AND_REFERAL_MODE == True:
...
async def check_plans_cmd(client, message):
    ...
    if await db.has_premium_access(user_id):         
        ...
        reply_markup = InlineKeyboardMarkup(btn)
        m=await message.reply_sticker("CAACAgIAAxkBAAIglGebvtPE5e_rzQ6YS8VHHQs9TatmAAKIGAAChWQpSLzyP8Qea7GeHgQ")         
